"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ChapterService = void 0;
const AuthUtil_1 = require("../util/AuthUtil");
class ChapterService {
    static upsertChapter(chapter, ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = yield AuthUtil_1.AuthUtil.verifyAndGetUser(ctx);
            try {
                const existingChapter = yield ctx.prisma.chapter.findUnique({
                    where: {
                        authorId: user.id,
                        id: chapter.id,
                    }
                });
                if (!existingChapter) {
                    return yield ChapterService.createChapter(chapter, ctx);
                }
                return yield ChapterService.updateChapter(chapter, ctx);
            }
            catch (e) {
                throw new Error(`Error upserting chapter from ChapterService.upsertChapter: ${e}`);
            }
        });
    }
    static createChapter(chapter, ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = yield AuthUtil_1.AuthUtil.verifyAndGetUser(ctx);
            try {
                const newChapter = yield ctx.prisma.chapter.create({
                    data: {
                        coverImageUrl: chapter.coverImageUrl,
                        chapterNumber: chapter.chapterNumber,
                        title: chapter.title,
                        volumeId: chapter.volumeId,
                        authorId: user.id,
                    }
                });
                return newChapter;
            }
            catch (e) {
                throw new Error(`Error creating chapter from ChapterService.createChapter: ${e}`);
            }
        });
    }
    static updateChapter(chapter, ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = yield AuthUtil_1.AuthUtil.verifyAndGetUser(ctx);
            try {
                const updatedChapter = yield ctx.prisma.chapter.update({
                    where: {
                        id: chapter.id,
                        authorId: user.id
                    },
                    data: {
                        coverImageUrl: chapter.coverImageUrl,
                        title: chapter.title
                    }
                });
            }
            catch (e) {
                throw new Error(`Error updating chapter from ChapterService.updateChapter: ${e}`);
            }
        });
    }
}
exports.ChapterService = ChapterService;
